...
Plugin URI: https://MichaelNeely.net
Description: The ultimate email list building plugin for WordPress. Capture new subscribers. Reward subscribers with a cusotm download upon opt-in. Build unlimites lists. Import and export subscribers easily with .csv
Version: 1.0
Author: Michael Neely
Author: https://MichaelNeely.net
License: GPL2
...
/* !1. HOOKS */
    ...
    // if(!defined('ACF_LITE') ) define('ACF_LITE', true); // turns off ACF
    ...
    // 1.8
    // hint: loads external scripts in WordPress admin
    ...
    // 1.10
    // hint: registeres actvate/deactivate/uninstall functions
    ...
    // 1.11
    // hint: triggers reward downloads
    add_action('wp', 'slb_trigger_reward_download');

/* !2. SHORTCODES */

// 2.1
// hintr: registers our custom shortcodes
...
function slb_form_shortcode($args, $content="") {
    ...
    $output = '
        ...
        <div class="slb">
            <form id="slb_register_form" name="slb_form" class="slb-form" method="post" action="/wp-admin/admin-ajax.php?action=slb_save_subscription" method="post">

                <input type="hidden" name="slb_list" value="'. $list_id .'">;

                '. wp_nonce_field( 'slb_register_subscription_'.$list_id, '_wpnonce', true, false ); 
                ...
                <p class="slb-input-container">
                    ...
                    //  including content in our form in our form html if content is passed into the function
                    if( strlen($content) ):
                    ...
                    if( $reward !== false ):
                        ...
                        // include messageabout rewqard
                        $output .='
                        <div class="slb-content slb-reward-message">
                            <p>Get a FREE DOWNLOAD of <strong>'. $reward['title'] .'</strong> when you subscribe to our list.</p>
                        </div>
                        ';
...
function slb_confirm_subscription_shortcode( $args, $content="" ) {
    ...
    // setup putput variable
    $output = '<div class="slb">';

    // setup email and list_id variables and handle if they are not defineed in teh scope
    $email = ( isset( $_GET['email'] ) ) ? esc_attr( $_GET['email'] ) : '';
    ...
    if( $subscriber_id && slb_validate_subscriber( $subscriber ) ):
        ...
        // IF subascriptions have no yet been added
        if( slb_validate_list( $list ) ):
            ...
            if( !slb_subscriber_has_subscription( $subscriber_id, $list_id ) ):
                ...
                if( !$optin_complete ):
                    ...
                    $output .= slb_get_message_html('Due to an unknown error, we are unable to confirm your subscription.' , 'error');
            ...
            // get confirmaton message html and append it to output
...
function slb_download_reward_shortcode( $args, $content="") {
    ...
    if( $reward === false):
        ...
        if($reward['downloads'] >= slb_get_option('slb_download_limit') ):
            ...
            $output .= slb_get_message_html( 'This link has erached it\'s download limit.', 'warning');
...
// 3.5
// hint: registeres custom plugin admin menus
...
function slb_save_subscription() {
    ...
    try {
        ...
        if( check_ajax_referer( 'slb-register-subscription_'.$list_id ) ):
            ...
            // attempt to create/savesubscriber
...
function slb_add_subscription ( $subscriber_id, $list_id ) {
    ...
    // IF the subscriber does NOT hve the current list subscription
    if( !slb_subscriber_has_subscription( $subscriber_id, $list_id ) ):
        ...
        // update slb_subscription_has_subscription
...
// expects form post data: $_POST['subscriber'] and $_POsT['list_id']
function slb_unsubscribe() {

    // setup deafault result data
    $result = array(
        'status' => 0,
    ...
    try {
        ...
        // setup success sttus and message
        $result['status']=1;
        $result['message']='Subscriptions updated. ';

        // get the updatesd list of subscriptions as html
...
function slb_confirm_subscription( $subscriber_id, $list_id ) {
    ...
    // IF subscription was savbed
    if( $subscription_saved ):

        // send email
        $email_sent - slb_send_subscriber_emil( $subscriber_id, $list_id );
...
function slb_add_reward_link( $uid, $subscriber_id, $list_id, $attachment_id ) {
    ...
    try {
        ...
        $table_name = $wpbd->prefix . "slb_reward_links";
...
function slb_trigger_reward_download() {
	...
	if( ! is_null($post) && $post->ID == slb_get_option( 'slb_reward_page_id') && isset($_GET['reward']) ):
		...
		if( $reward !== false && $reward['downloads'] < slb_get_option( 'slb_download_limit') ):
            ...
            // get teh reward mimetype
            $mimetype = $reward['file']['mime_type'];
            // extract the filetype form the mimetype
            $mimetype_array = explode('/', $mimetype);
            $filetype = $mimetype_array[1];
		
			header("Content-type: ".$reward['file']['mime_type'],true,200);
...
// 5.12 
// hint: increases reward download link count by one
function slb_update_reward_link_downloads( $uid ) {
    global $wpdb;

    // setuyp our return value
    $return_value = false;
    ...
    try {
        ...
        // update downloads for htis reward link entry
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE $table_name
...
// 5.13 
// hint: generates a .csv file of subscriber's data
...
function slb_download_subscribers_csv() {
    ...
    // get the lists's subscribers or get all subscribers if no list id is given
    ...
    if( $subscribers !== false ):
        ...
        // append $csv_heaaders to out csv file
        ...
        foreach( $subscribers as &$subscriber_id ):
            ...
            // remove the subscriptions and name columns formj the data
            unset($subscriber_data['subscriptions']);
            unset($subscriber_data['name']);
        ...
        // read our new csv file and store itr's contents in $fc
...
// 5.14
// hint: this function retrieves a csv file from the server and parses the dara into a php array
...
function slb_parse_import_csv() {
    ...
    try {
        ...
        // get attachment id from $_POST['slb_import_file_id']
        $attachment_id = (isset($_POST['slb_import_file_id'])) ? esc_attr( $_POST['slb_import_file_id'] ) : 0;

        // get the file\name using wp's get_attached_file
        $filename = get_attached_file( $attachment_id );

        // IF we got the filename
        if( $filename !== false):

            // parse the csv file into a php arrray using slb_csv_to_array
            ...
            // return an error iof we could not retrieve the file
...
// 5.15
// hint: imports new subscribers from okur import admin ppage
// this functrion is a form handler and expect subscriber data in the $_POST scope
...
// 5.17
// hint: checks the version of WordPRess and displays a medssage in teh pluginpage if the version is untested.
function slb_check_wp_version() {
    ...
    if ( $pagenow == 'plugins.php' && is_plugin_active('snappy-list-builder/snappy-list-builder.php') ):
        ...
        // tested versions
        // these are thge versions we've tested in our plugin
...
// 5.18
// hint: runs function for plugin uninstall
...
function slb_remove_plugin_tables() {
    ...
    try {
        ...
        // delete table removed from database
...
// 5.20
// hint removes plugin related post data
...
function slb_remove_post_data() {
    ...
    try {
        ...
        // remove data from the posts db table where post types are equal t our custom post types
...
function slb_subscriber_has_subscription( $subscriber_id, $list_id ) {
    ...
    if( in_array($list_id, $subscriptions) ):
        ...
        //  this subscriber is already isubscribed to this list
        $has_subscription = true;

    else:

        // did not find $list_id in $subscriptions 
        // this subscriber is not yet a subscriber to theis list
    endif;
...
// 6.5
// hint: gets the unique acf key from the field key
function slb_get_acf_key( $field_name ) {
    $field_key = $field_name;

    switch( $field_name ) {
        case 'slb_fname':
            $field_key = 'field_5d0eb939cf5f7';
            break;
        case 'slb_lname':
            $field_key = 'field_5d0eb9782e367';
            break;
        case 'slb_email':
            $field_key = 'field_5d0eb99005312';
            break;
        case 'slb_subscriptions':
            $field_key = 'field_5d0eb9afd559c';
            break;
        case 'slb_enable_reward':
            $field_key = 'field_5d1d68aa19943';
            break;
        case 'slb_reward_title':
            $field_key = 'field_5d1d693719944';
            break;
        case 'slb_reward_file':
            $field_key = 'field_5d1d697a19945';
            break;
...
function slb_get_page_select( $input_name='slb_page', $input_id="", $parent=-1, $value_field="id", $selected_value="") {
    ...
    foreach($pages as $page):
        ...
        // get teh page id as our default option value
        $value = $page->ID;
        ...
        switch( $value_field ){
            ...
            // check if this option is currently slelected option
            $selected = '';
...
function slb_get_manage_subscriptions_html( $subscriber_id ) {
    ...
    try {
        ...
        // get array of list_ids for the subscriber
        ...
        $output = '
            <form id="slb_manage_subscriptions_form" class="slb-form" method="post" action="/wp-admin/admin-ajax.php?action=slb_unsubscribe">
	...
	if( !slb_validate_list( $list ) || !slb_validate_subscriber( $subscriber ) ):
        ...
        if( $reward !== false ):
            ...
            switch( $email_template_name ) {
                ...
                case 'new subscription':
                    // set reward text
                    $reward_text = '<p>After confirming your subscription, we will send you a link a FREE DOWNLOAD of '.$reward['title'] .'</p>';
                    break;
                case 'subscription_confimed':
                    // get download limit
		...
		// setup email templates
            ...
            $email_templates['new_subscription'] = array(
                ...
                <p>Please <a href=""'. $optin_link .'">Click here to confirm yuour subscription.</a></p>
                ' . $reward_text . $default_email_footer . $unsubscribe_text,
            );

            // template: subscription cofirmed
            $email_tempates['subscription_confirmed']=array(
...
function slb_get_optin_link( $email, $list_id=0 ) {
    ...
    try {
        ...
        // get character to start querystring
        $startquery = slb_get_querystring_start( $permaliink );
...
// 6.19 
// hint: returns false if list has no reqard or retuirns the object containing file and title if it dies
    ...
    try {
        ...
        if( $uid && $reward !== false ):
            ...
            if( $link_added === true ):
                ...
                // get character to start querystirng
                $startquery = slb_get_querystring_start( $permalink );
...
function slb_generate_reward_uid( $subscriber_id, $list_id ) {
    ...
    // IF subscriber and list are balid
...
function slb_get_reward( $uid ) {
    ...
    // get downloads form reward link
...
function slb_get_list_subscribers( $list_id=0 ) {
    ...
    if( slb_validate_list( $list )):
        ...
        $subscribers_query = new WP_Query(
            ...
            // IF $subscribers_query isser and query returns reuslts
...
// 6.24
// hint: returns the number of subscribers in this list
...
// 6.25 
// hint: returns a unique link for downloading a subscrubers csv
...
// 6.26
// hint: this function reads a csv file and converts the contents to php array
function slb_csv_to_array($filename='', $delimiter=',') {

    // this is an important setting!
    ini_set('auto_detect_line_endings', true);

    // IF the file doisn'e exost or the file ins not readable return false
    ...
    while(($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
            // count the nunmber of items in this data
            $num = count($data);
...
// 6.28
// hint: gets an array of plugin option data (group and settings) so as to save it all in one place
function slb_get_options_settings() {

    // setup our return data
    $settings = array(
        'group' =>'slb_get_options_settings',
        'settings' =>array(
...
function slb_dashboard_admin_page() {
    ...
    $output = '
        <div class="wrap">
            ...
            <p>The ultimate email list building plugin for WordPress. Capture new subscribers. REward subscribers with a custom download upon o[pt-in. Build unlimited lists. Import and export subscribers easily with .csv</p>
...
// 8.2
// hint: import subcribers admin page
function slb_import_admin_page() { 
    ...
    echo('
        ...
        <div class="wrap" id="import_subscribers">
            ...
            <form id="import_form_2" method="post" action="/wp-admin/admin-ajax.php?slb_import_subscribers">
                <table class="form-table">
